...
public class SentenceBuilder {
    ...
    public void describeClass() {
        ...
        for(ClassType classType : diagram.getClasses()) {
            ...
            DescribeClassAttributes(classType);
    ...
    public void DescribeClassAttributes(ClassType classType) {
        ...
        for(Connector connector : classType.getConnectors()) {
            if((connector.getType().equals("Realisation")) && (connector.getSourceType().equals("UseCase") ||
                ...
                if(connector.getType().equals("Generalization")) {
                    for(var entry : diagram.getGeneralizationSets().entrySet()) {
                        if (entry.getValue().getConnectors().containsKey(connector.getConnectorId())) {
                            entry.getValue().getConnectors().put(connector.getConnectorId(), connector);
                        }
                    }
                }
    ...
    private void addConnectorComments(ClassType classType, Connector connector) {
        ...
        for(var entry : diagram.getGeneralizationSets().entrySet()) {
            ...
            for (var connector : generalizationSet.getConnectors().entrySet()) {

                if (generalizationClass.equals("")) {
                    generalizationClass = connector.getValue().getEndName();
                }
                subtypes += connector.getValue().getSourceName() + ", ";
    ...
    public String relationshipAssociationDescriptionSentence(String source, String target,
                                                             String sourceRole, String targetRole) {


          if (!(sourceRole.equals("")) && !(targetRole.equals(""))) {
              return String.format(phrases.getDescribeEmptyMultiplicity(), source,
                      String.format(phrases.getDescribeRole(), sourceRole), target,
                      String.format(phrases.getDescribeRole(), targetRole));
          }
          else if (!(sourceRole.equals(""))) {
    ...
    public String relationshipAggregationDescriptionSentence(String descriptiveSentence, String source, String target,
                                                             String sourceRole, String targetRole) {


        if (!(sourceRole.equals("")) && !(targetRole.equals(""))) {
            return String.format(descriptiveSentence, source,
                    String.format(phrases.getDescribeRole(), sourceRole), target,
                    String.format(phrases.getDescribeRole(), targetRole));
        }
        else if (!(sourceRole.equals(""))) {
